...
func NewPSQL(options OptionsPSQL) (*UserRepoPSQL, error) {
	...
	if err := createRoleTable(options.Conn, table); err != nil {
		return nil, err
	}
...
func createRoleTable(conn *pgx.Conn, table string) error {
	query := fmt.Sprintf(`
    CREATE TABLE IF NOT EXISTS roles (
		id TEXT PRIMARY KEY,
        ownerid TEXT FOREIGN KEY REFERENCES %s(id),
        role TEXT NOT NULL,
    );
    `, table)
	_, err := conn.Query(context.Background(), query)
...
func (u *UserRepoPSQL) ListUsers(ctx context.Context) ([]user.User, error) {
	...
	for rows.Next() {
		...
		roleQuery := "SELECT * FROM roles WHERE ownerid=$1"
		rows, err := u.conn.Query(ctx, roleQuery, user.Id)
		if err != nil {
			return nil, err
		}
		for rows.Next() {
...
func (u *UserRepoPSQL) GetUser(ctx context.Context, id string) (*user.User, error) {
	...
	roleQuery := "SELECT * FROM roles WHERE ownerid=$1"
	rows, err := u.conn.Query(ctx, roleQuery, id)
	if err != nil {
		return nil, err
	}
	for rows.Next() {
